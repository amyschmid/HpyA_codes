---
title: "2021Combined_hpyAsalt_newcounts-AKS"
...
suppressPackageStartupMessages(library(bazar))
...
#Note: see Supplementary Note 1 for details on analysis pipeline. In brief, two analyses were run for each contrast: one with 4 outlier samples removed, one with 5. Here we show analysis with 4 outlier removed. Use the same code included here for 5 outliers removed but with input file "combineddata_5reps.csv". Then use the function combindiffexpr() (code chunk starting at line 337 below).

mycount <- as.matrix(read.csv("combineddata_21samples.csv", row.names=1))
metafile <- read.csv("combinedmeta_21samples.csv", row.names=1) 
...
#Steps here follow the same logic as those above, except that the comparison is for the effect of genotype alone (rather than condition*genotype interaction) in a given media condition (low salt).
...
### The effect of low salt on Hbt salinarum parent strain (here considered wild type, "WT")
```{r lowsaltinWT, warning=FALSE}
#Steps here follow the same logic as those above, except that the comparison is for the effect of condition alone (rather than condition*genotype interaction or genotype) in a given genotype (WT).
...
### combining the results of this analysis (4 outlier samples removed) with that of 5 outlier samples removed (same code, different input files)
```{r}
#set up the function to find the overlap between the two lists of differentially expressed genes

combinediffexpr=function(diffexpr1,diffexpr2) {
  #combine deseq2 output from 2 different tests - only output those genes in input 1 that are also present in input 2
  #use log fold change and padj values from input 1
  
 
  
  l=length(diffexpr1$Locus)
  decisions=vector(length=l)
  
  for (i in (1:l)) {
    relgene=diffexpr1$Locus[i]
    a=which(diffexpr2$Locus==relgene)
    if (!is.empty(a)) {
      decisions[i]=TRUE
    }
  }
  
  diffexpr1=diffexpr1[decisions,]
}
```

```{r}
# use the function, here with the example of differential expression results for the WT low salt : WT optimal salt ratios
	WT_salteffect_5 <- read_csv("WT_salteffect_5.csv")
	WT_salteffect_21 <- read_csv("WT_salteffect_21.csv")
	WT_salt_21_5=combinediffexpr(WT_salteffect_21,WT_salteffect_5)
write.csv(WT_salt_21_5,row.names=FALSE,"WTsalt_21_5_combineed.csv")

```

### 2D dotplot of expression ratios for each contrast
...
### Venn diagram for intersection between ko DEGs in low salt and optimal salt
```{r}
#make gene lists for each condition
ko.comb <- read.delim ("June2021-TableS4-forR.txt", sep = "\t")
lowgenes <- subset(ko.comb, Condition == "Low" | Condition == "Optimal_low")
optgenes <- subset (ko.comb, Condition == "Optimal" | Condition == "Optimal_low")

#find set sizes
#low salt
length(lowgenes$Gene)
#optimal salt
length(optgenes$Gene)
#WT
length(WT_salt_21_5$Locus)
#WT and low salt
length(intersect(lowgenes$Gene, WT_salt_21_5$Locus))
#WT and optimal salt
length (intersect(optgenes$Gene, WT_salt_21_5$Locus))
#optimal and low
length(intersect(optgenes$Gene,lowgenes$Gene))
#all three conditions
length(intersect(WT_salt_21_5$Locus, intersect(optgenes$Gene,lowgenes$Gene)))

#draw the VennDiagram depending on the results
temp<-draw.triple.venn(143, 46, 881, 22, 14, 43,7, c("∆hpyA:WT low salt", "∆hpyA:WT optimal salt", "WT low:optimal"), col= c("white","white", "white"),fill=c("light coral", "dark red", "cadetblue"), alpha = rep(0.5, 2, 0.5), cex=3, cat.cex=2, ext.pos=c(315, 45, 270), margin=0.25, euler.d = TRUE)
grid.draw(temp)
```
